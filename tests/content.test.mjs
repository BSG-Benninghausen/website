...
export default async function run(api, ck) {
  ...
  ck("13 Berechtigungen", d.items.length === 13 && d.items.some((p) => p.key === "manage_news") && d.items.some((p) => p.key === "manage_team") && d.items.some((p) => p.key === "view_finance") && d.items.some((p) => p.key === "manage_club"));
